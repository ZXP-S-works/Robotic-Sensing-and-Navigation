...
% Load images.
% buildingDir = './3Cinder_BW_50%_x6/Calibrated/';
buildingDir = './2Latino_SC_50%_x6/Calibrated/';
...
% points = SURFPoints(Location) constructs a SURFPoints object from an 
% M-by-2 array of [x y] point coordinates, Location.
...
panorama = zeros([height width rgb_num], 'like', I);
...
xLimits = [xMin-100 xMax+400];
yLimits = [yMin-150 yMax+100];
...
% % Convert all the tranforms with respect to the first image.
% Tinv = invert(tforms(centerImageIdx));
% 
% for i = 1:numel(tforms)    
%     tforms(i).T = tforms(i).T * Tinv.T;
